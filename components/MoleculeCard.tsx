...
import {
    ...
    FileText, Percent, TestTube, SlidersHorizontal,
} from "lucide-react";
...
const getScoreRingStyle = (score: number) => {
    if (score >= 70) return "border-emerald-500 text-emerald-400";
    if (score >= 45) return "border-yellow-500 text-yellow-400";
                            ...
                            >
                                <td className="px-3 py-2">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        {ep.hard_fail && (
                                            <span className="text-[9px] uppercase tracking-wider text-red-500 border border-red-900 rounded px-1 py-0.5">
                                                hard fail
                                            </span>
                                        )}
                                        {ep.interpretation && (
    ...
    return (
        <div className="rounded-lg bg-gray-800/30 border border-gray-700 overflow-hidden">
            ...
            <div className="p-3 space-y-4">
                ...
                {/* Full endpoint table (collapsible) */}
                {hasEndpointTable && (
                    <div>
                        <button
                    ...
                    <div className="flex items-center gap-1.5 mt-2 overflow-x-auto">
                        {step.schematic.split("→").map((part, i, arr) => (
                            <div key={i} className="flex items-center gap-1.5 flex-shrink-0">
                                <code
                                    className="font-mono text-[10px] text-teal-400 bg-gray-800 px-2 py-1 rounded border border-gray-700 max-w-[180px] truncate block"
                                    title={part.trim()}
                                >
                                    {part.trim()}
                                </code>
                        ...
                        <div>
                            ...
                            <div className="space-y-1">
                                {step.starting_materials.map((smi, i) => (
                                    <code
                                        key={i}
                                        className="block font-mono text-[10px] text-teal-400 bg-gray-800 px-2 py-1 rounded border border-gray-700 truncate"
                                        title={smi}
                                    >
                                        {smi}
                                    </code>
...
export default function MoleculeCard({ compound, jobId, index }: MoleculeCardProps) {
    ...
    const ringStyle = getScoreRingStyle(score);
    ...
    const handleExpand = async () => {
        ...
        if (!expanded && !breakdown) {
            ...
            try {
                ...
                // non-critical — score breakdown is optional
            } finally {
        ...
        <div className="card-hover animate-fade-in">
            ...
            {expanded && (
                <div className="mt-4 pt-4 border-t border-gray-800 space-y-5 animate-slide-up">
                                ...
                                <div className="space-y-1.5">
                                    {[
                                        ...
                                        <div key={label} className="flex justify-between text-xs items-center">
                                            ...
                                            <span className={ok ? "text-emerald-400" : "text-yellow-400"}>{String(value)}</span>
                                        </div>
                                    ))}
                    ...
                    {/* Full ADMET report — narrative + endpoint table + thresholds */}
